...
description: 根据用户自然语言输入生成映射表（dim/dm/ads）相关的 SQL 查询语句的技能。当用户需要查询维度枚举值、生成映射表 SQL、进行多表关联查询、或基于中间表进行数据分析时使用此技能。支持三个业务线（浏览器、浏览器信息流、内容中心），覆盖维度枚举值查询、指标聚合查询、多表关联查询三种查询类型；并包含两个专题子域：浏览器 Push（推送送达/点击/CTR/Push DAU/真实送达/关闭率）与浏览器信息流都江堰（有效用户DAU/有效时长/消费UV/双频道CTR/小说短故事阅读）。触发场景：当用户明确提到 "执行 nl-mapping-table-sql"、"使用 nl-mapping-table-sql" 时，必须且只能使用本技能；用户提到"映射表"、"中间表"、"维度枚举"、"dim表"、"dm表"、"ads表"、"关联查询"、"push/推送指标"、"送达点击CTR"、"Push DAU"、"都江堰"、"信息流有效用户/有效时长"等也要使用本技能。
---
...
> **专题子域优先路由**：先判断是否命中以下专题，命中则改读专题文件（口径基于专属底表，与通用 dm/raw 模板不同）：
> - **Push / 推送**（送达、点击、CTR/UTR、Push DAU、真实送达、push 时长/消费、试探物料、关闭率）→ `reference/browser/push-metrics-reference.md` + `reference/browser/push-table-schema.md`
> - **都江堰 / 信息流有效口径**（有效用户DAU、有效时长、消费UV/VV、信息流DAU、双频道CTR/UTR、小说/短故事阅读、自建vs火山）→ `reference/browser-feed/dujiangyan-metrics-reference.md`
> - **信息流双端**（浏览器信息流+内容中心的统一有效用户视角；双端合计、跨端对比、双端有效用户 DAU/VV/时长）→ `reference/feed-dual-end/concept.md` + `reference/feed-dual-end/valid-user-metrics-reference.md`
> - 这三个专题底表 `date` 为整型，SQL 中必须用不带引号的整型（`date = 20260628`），否则分区裁剪失效导致超时。
...
### 专题指标参考文件（特定业务子域，底表/回调日志层）
> 这些子域基于专属底表，查询模式与通用 dm/raw 模板不同，命中对应关键词时优先读取。

- 浏览器 Push 核心指标 SQL 口径：`reference/browser/push-metrics-reference.md`
  - **触发**：push、推送、送达、点击率/CTR/UTR、Push DAU、真实送达、push 时长、push 消费、试探物料、动态关闭率
- 浏览器 Push 表结构 / 维度 / 关联模式：`reference/browser/push-table-schema.md`
  - **覆盖**：`push_callback_log` 等底表；维度 expid / pushId类型 / 物料队列 / 体裁 / 用户分层 / 画像
- 浏览器信息流 都江堰核心指标（Label 宽表）：`reference/browser-feed/dujiangyan-metrics-reference.md`
  - **触发**：都江堰、有效用户DAU、有效时长、消费UV/VV、信息流DAU、双频道CTR/UTR、小说/短故事阅读时长、自建/火山实验对比
