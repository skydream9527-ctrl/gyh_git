...
def test_messages_to_openai_assistant_with_tool_use():
    ...
    # User message with empty content is always added (zhipuai/glm fix)
    user_msg = out[2]
...
def test_messages_to_openai_filters_empty_assistant():
    """Empty assistant messages (no text, no tool calls) should be filtered out."""
    msgs = [
        {"role": "assistant", "content": [{"type": "text", "text": ""}]},
        {"role": "user", "content": "hello"},
    ]
    out = _messages_to_openai(msgs, system_prompt="SYS")
    assert len(out) == 2  # system + user (assistant filtered out)
    assert out[0]["role"] == "system"
    assert out[1]["role"] == "user"
    assert out[1]["content"] == "hello"


def test_messages_to_openai_user_only_tool_results():
    """User messages with only tool_result blocks should still add a user message."""
    msgs = [
        {
            "role": "user",
            "content": [
    ...
    out = _messages_to_openai(msgs, system_prompt="SYS")
    assert len(out) == 3  # system + user (empty) + tool
    assert out[1]["role"] == "user"
...
def test_preset_models_contains_9():
    ...
    assert "zhipuai/glm-5.2" in ids
    assert "xiaomi/glm-5" in ids
    assert "xiaomi/mimo-v2.5-pro" in ids
    assert len(PRESET_MODELS) == 9


def test_route_resolution():
    from app.services.llm.llm_gateway import _route
    ...
    # Note: zhipuai/glm-5.2, xiaomi/glm-5, vertex_ai/* route to openai_chat
    # only when gateway_enabled=True (runtime config dependent)
