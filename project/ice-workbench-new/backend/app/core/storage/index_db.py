...
@lru_cache
def get_index_db() -> IndexBackend:
    """Factory: returns the configured index backend (default: sqlite).

    Controlled by ICE_INDEX_BACKEND in .env. Currently only 'sqlite' is
    implemented; the Protocol exists so a Postgres (or other) backend can be
    dropped in without touching any of the 17 calling modules.
    """
    settings = get_settings()
    backend = getattr(settings, "ICE_INDEX_BACKEND", "sqlite")
    if backend != "sqlite":
        raise RuntimeError(
            f"Unsupported ICE_INDEX_BACKEND={backend!r}. "
            "Only 'sqlite' is currently implemented."
        )
    return IndexDB(settings.cache_db_path)
