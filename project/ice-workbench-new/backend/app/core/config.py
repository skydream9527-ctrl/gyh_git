...
# Default sensitive-pattern set for the memory write-time scanner. Comma-separated
# `category:regex` items; each item's category is the substring before the first
# ':' and the regex is everything after it. Regexes intentionally avoid commas so
# the comma split stays unambiguous. Covers common secret shapes plus CN aliases.
_DEFAULT_MEMORY_SENSITIVE_PATTERNS = (
    r"api_key:(?i)(api[_-]?key|secret[_-]?key)\s*[:=]\s*\S+,"
    r"token:(?i)(access[_-]?token|auth[_-]?token|bearer\s+token)\s*[:=]?\s*\S+,"
    r"password:(?i)(password|passwd|pwd)\s*[:=]\s*\S+,"
    r"secret:(?i)secret\s*[:=]\s*\S+,"
    r"private_key:-----BEGIN [A-Z ]*PRIVATE KEY-----,"
    r"密钥:(?i)(密钥|私钥)\s*[:=：]?\s*\S+"
)
...
class Settings(BaseSettings):
    ...
    ICE_INDEX_BACKEND: str = "sqlite"  # only 'sqlite' implemented; Protocol allows future swap
    # When True, refresh tokens are delivered via httpOnly + Secure + SameSite=Strict
    # cookie instead of the JSON response body. Access tokens remain in the body
    # (stored in memory/localStorage by the SPA). This hardens against XSS token
    # theft at the cost of slightly more complex auth flow. Default off for
    # backward compatibility; flip on per-env after testing cookie forwarding
    # in your proxy/dev setup.
    ICE_REFRESH_COOKIE_ENABLED: bool = False
    ...
    # When True, the user-facing ws loop runs on the shared agent_kernel
    # (the same loop the sub-agent runtime uses) via WebSocketEventSink instead
    # of ws.py's inline copy. Default off so the streaming path is bit-stable on
    # upgrade; flip on per-install to A/B the unified kernel on staging, then
    # delete the legacy inline loop once validated.
    ICE_AGENT_KERNEL_ENABLED: bool = False
    ...
    # Conditional tool retry (structured-tool-error-handling). When enabled,
    # a failed tool call whose classifier-derived `recoverable` is True is
    # retried at most once after a fixed backoff. Default off, aligning with
    # the ICE_*_ENABLED convention so existing runtime behavior is unchanged.
    ICE_TOOL_RETRY_ENABLED: bool = False
    ICE_TOOL_RETRY_BACKOFF_SEC: float = 1.0
    ...
    # Memory timestamp/decay/safety. Drives the memory write-time sensitive
    # scanner and the freshness decay applied during memory ranking/loading.
    # All follow the ICE_* convention with overridable defaults.
    ICE_MEMORY_HALF_LIFE_DAYS: float = 30.0
    ICE_MEMORY_STALENESS_DAYS: float = 90.0
    ICE_MEMORY_LEGACY_DECAY: float = 0.5
    # Comma-separated "category:regex" list; the category is the text before the
    # first ':' and the regex is the remainder. Invalid regexes are skipped with
    # a warning (see memory_sensitive_patterns) so one bad item never blocks all
    # writes. Regexes avoid commas to keep the split unambiguous.
    ICE_MEMORY_SENSITIVE_PATTERNS: str = _DEFAULT_MEMORY_SENSITIVE_PATTERNS
    ...
    @property
    def memory_sensitive_patterns(self) -> list[tuple[str, re.Pattern]]:
        """Parse ICE_MEMORY_SENSITIVE_PATTERNS into compiled (category, regex)
        pairs. Each comma-separated item is split on its first ':' into a
        category and a regex. Items with no ':' or an invalid regex are skipped
        with a warning so a single bad config entry never blocks all writes."""
        compiled: list[tuple[str, re.Pattern]] = []
        for raw in self.ICE_MEMORY_SENSITIVE_PATTERNS.split(","):
            item = raw.strip()
            if not item or ":" not in item:
                if item:
                    _logger.warning("skipping malformed sensitive pattern (no category): %r", item)
                continue
            category, pattern = item.split(":", 1)
            category = category.strip()
            pattern = pattern.strip()
            if not category or not pattern:
                _logger.warning("skipping malformed sensitive pattern: %r", item)
                continue
            try:
                compiled.append((category, re.compile(pattern)))
            except re.error as exc:
                _logger.warning("skipping invalid sensitive regex for category %s: %s", category, exc)
        return compiled
@lru_cache
