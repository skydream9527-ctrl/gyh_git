    ...
    qs = urlencode(
        {
            ...
            "state": state,
            "scope": (
                "contact:user.id contact:user.base contact:user.email "
...
async def exchange_code(code: str) -> dict:
    ...
    plus `feishu_access_token`, `feishu_refresh_token`, and token expiry
    info so the backend can later call Feishu APIs on the user's behalf.
    """
    assert_configured()
    s = get_settings()
    now = time.time()
    ...
    return {
        ...
        "feishu_access_token": user_token,
        "feishu_refresh_token": data.get("refresh_token", ""),
        "feishu_token_expires_at": now + (data.get("expires_in") or 7200),
        "feishu_refresh_expires_at": now + (data.get("refresh_token_expires_in") or 2592000),
    }
