    ...
    if imported_dir.exists():
        ...
        if imp_meta_dir.exists():
            for mf in sorted(imp_meta_dir.glob("*.json")):
                ...
                fname = m.get("filename") or "imported.md"
                # 同时输出 FileMeta 标准键（id/name/size_bytes/path/format）+
                # 兼容旧字段（file_id/filename/size），让前端按 id/name 渲染、按
                # file_id ?? id 调 API 都能 work。
                items.append({
                    "id": m["file_id"],
                    "file_id": m["file_id"],
                    "name": fname,
                    ...
                    "scope": "imported",
                    "task_id": task_id,
                    "format": _detect_format(fname),
                    "file_type": _detect_type(_detect_format(fname)),
                    "path": str(body_path.relative_to(paths.root)) if body_path.exists() else "",
                    "is_pinned": False,
...
async def list_public_files() -> list[dict]:
    """Public files area = platform-generated builtin docs FIRST, then any
    admin-dropped files in `files/` (excluding sources that the builtin
    generator already consumes — 使用指南.md）.
    """
    paths = get_paths()
    out: list[dict] = []
    # 1) Platform read-only docs (guide + per-agent + per-skill)
    try:
        out.extend(builtin_docs_svc.list_docs())
    except Exception:
        # enumeration is best-effort — never let a bad agent.json hide the page
        pass
    # 2) Any other admin-placed files in files/ — skip sources already surfaced
    #    through builtin docs (the guide) to avoid duplication.
    if paths.files.exists():
        hidden = {"使用指南.md", "平台使用指南.md"}
        for p in sorted(paths.files.iterdir()):
            if p.is_dir() or p.name.startswith(".") or p.name in hidden:
                continue
            try:
                stat = p.stat()
            except OSError:
...
async def read_file_text(task_id: str | None, file_id: str, *, owner_id: str | None = None) -> dict:
    ...
    if task_id:
        ...
        # 兜底：导入文件（imported）走另一条 meta 目录。
        imp_meta = read_json(paths.task_files_imported_meta(task_id, file_id))
        if imp_meta:
            fpath = paths.task_files_imported(task_id) / f"{file_id}.md"
            if not fpath.exists():
                raise APIError(404, ErrorCode.RESOURCE_NOT_FOUND, "文件已被删除")
            return {
                "meta": {**imp_meta, "name": imp_meta.get("filename")},
...
async def delete_public_file(file_id: str) -> None:
    paths = get_paths()
    meta_path = _public_meta_path(file_id)
    if builtin_docs_svc.is_builtin(file_id):
        raise APIError(400, ErrorCode.PERMISSION_DENIED, "平台内置文档只读，不能删除")
    meta = read_json(meta_path)
    if not meta:
        for m in await list_public_files():
            if m["id"] == file_id:
...
async def read_public_file(file_id: str) -> dict:
    # Short-circuit for platform-generated read-only docs.
    if builtin_docs_svc.is_builtin(file_id):
        return builtin_docs_svc.read_doc(file_id)
    paths = get_paths()
...
async def delete_task_file(task_id: str, file_id: str) -> None:
    ...
    # 兜底：导入文件（imported）走 imported meta 目录 + .md 实体。
    imp_meta_path = paths.task_files_imported_meta(task_id, file_id)
    imp_meta = read_json(imp_meta_path)
    if imp_meta:
        fpath = paths.task_files_imported(task_id) / f"{file_id}.md"
        try:
            fpath.unlink()
            ...
            pass
        try:
            imp_meta_path.unlink()
        except OSError:
            pass
        # 计数器回退
        task_meta_path = paths.task_meta(task_id)
        tmeta = read_json(task_meta_path) or {}
        tmeta["imported_file_count"] = max(0, int(tmeta.get("imported_file_count", 0)) - 1)
...
async def import_external(
    *,
    ...
    if source_type == "feishu_doc":
        from app.services.integration import feishu_import_svc
        ...
        from app.services.integration import kb_svc
        ...
        title = article.get("title") or "KB 文章"
        body = article.get("content") or article.get("body") or ""
        ref = {**source_ref, "url": article.get("url")}
        if not source_url:
            source_url = article.get("url") or f"kb://{source_ref['kb_id']}/{source_ref['article_id']}"
    else:
    ...
    # 与 list_task_files 的导入分支保持同形：标准 FileMeta 键 + 兼容旧字段。
    return {
        **meta,
        "id": file_id,
        "name": filename,
        ...
        "file_type": _detect_type(_detect_format(filename)),
        "path": str(fpath.relative_to(paths.root)),
        "is_pinned": False,
        "created_at": meta["imported_at"],
    }
    ...
    *, task_id: str, file_id: str, user_id: str, is_owner: bool, is_admin: bool,
    ...
    try:
        if source_type == "feishu_doc":
            ...
            body = art.get("content") or art.get("body") or ""
        else:
