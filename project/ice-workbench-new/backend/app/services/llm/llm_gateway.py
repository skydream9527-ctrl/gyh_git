...
def _messages_to_openai(messages: list[dict], system_prompt: str) -> list[dict]:
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # (zhipuai/glm rejects empty assistant messages with "输入不能为空")
            text_content = "".join(text_parts)
            if not text_content and not tool_calls:
                continue
            entry: dict = {"role": "assistant", "content": text_content or None}
            if tool_calls:
        ...
        if role == "user":
            ...
            # Always add a user message even if text is empty — zhipuai/glm
            # requires at least one user message or rejects with error 1214
            # "输入不能为空". Use empty string if no text blocks present.
            out.append({"role": role, "content": "".join(text_parts2) or ""})
...
async def _stream_openai_chat(
    *,
    ...
    # stream_options is OpenAI-specific; zhipuai/glm and other providers
    # may reject it with 400. Only include for azure_openai models.
    if model.startswith("azure_openai/"):
...
def normalize_tool_outcome(outcome: dict) -> dict:
    ...
    if outcome.get("success") and isinstance(outcome.get("result"), dict):
        ...
        if code:
            cls = error_classifier.classify(code)
            return {
                "success": False,
                "status": "error",
                "error": {
...
def _emit_retry_event(
    audit_ctx: dict | None, code: str | None, error_type: str | None, *, attempt: int
) -> None:
    """Record one Audit_Log event for an automatic tool retry.

    Routes through the project's existing ``event_log`` channel. ``event_log``
    discards events without a ``task_id`` and never raises, but we still guard
    defensively so audit failures can never abort the retry path (R4.8).
    """
    ctx = audit_ctx or {}
    try:
        event_log.emit(
            task_id=ctx.get("task_id"),
            conv_id=ctx.get("conversation_id") or ctx.get("conv_id"),
            run_id=ctx.get("run_id") or ctx.get("subagent_run_id"),
            source="retry_controller",
            event_type="tool_retry",
            level="WARN",
            code=code,
            message=f"auto-retry after recoverable tool failure (attempt {attempt})",
            payload={"error_code": code, "error_type": error_type, "attempt": attempt},
        )
    except Exception:  # pragma: no cover - audit must never break the loop
        log.warning("retry audit emit failed code=%s", code)


async def run_tool_with_retry(
    coro_factory,
    *,
    timeout: int = TOOL_TIMEOUT_SEC,
    settings=None,
    audit_ctx: dict | None = None,
) -> dict:
    """Execute a tool with at most one conditional automatic retry.

    Shared by the main loop (``ws.py``) and the sub-agent loop
    (``agent_runtime.run_agent_turn``) so both apply identical retry logic
    (R4.10). Behaviour:

    * Always run once and normalize the outcome.
    * If the Retry_Gate (``ICE_TOOL_RETRY_ENABLED``) is off, return immediately
      — i.e. behaviour is identical to a single ``run_tool_with_timeout``
      (R4.1 / R7.4).
    * On success, or on a failure whose ``recoverable`` is False, return the
      first outcome without retrying (R4.4).
    * On a recoverable failure, emit an audit event, wait a fixed backoff
      (``ICE_TOOL_RETRY_BACKOFF_SEC``), then retry exactly once and return the
      retry's normalized outcome (R4.3/4.5/4.6/4.7/4.8).

    The retry is transparent to the LLM and does NOT advance the ReAct round
    counter — it happens entirely inside one tool execution. The retry block is
    wrapped in ``try`` so any retry-side exception degrades to returning the
    first failure outcome, never masking the original error.
    """
    s = settings if settings is not None else get_settings()

    outcome = normalize_tool_outcome(await run_tool_with_timeout(coro_factory, timeout=timeout))

    if not getattr(s, "ICE_TOOL_RETRY_ENABLED", False):
        return outcome
    if outcome.get("success"):
        return outcome

    err = outcome.get("error") or {}
    if not err.get("recoverable"):
        return outcome

    try:
        _emit_retry_event(audit_ctx, err.get("code"), err.get("error_type"), attempt=1)
        await asyncio.sleep(getattr(s, "ICE_TOOL_RETRY_BACKOFF_SEC", 1.0))
        return normalize_tool_outcome(
            await run_tool_with_timeout(coro_factory, timeout=timeout)
        )
    except Exception:
        # Retry machinery itself failed (e.g. backoff cancelled): never mask
        # the original failure — fall back to the first outcome.
        return outcome
...
PRESET_MODELS = [
    ...
    {"id": "zhipuai/glm-5.2", "label": "GLM-5.2", "input_unit_price": 1.0, "output_unit_price": 4.0},
