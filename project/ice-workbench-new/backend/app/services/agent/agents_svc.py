...
# ---- CC-style declarative config getters ----
#
# P1 (agent-runtime-consolidation): these are now thin wrappers over
# `AgentConfig`, which centralizes every normalization / camelCase-alias /
# default rule in one `model_validator`. Behavior is byte-for-byte identical to
# the pre-refactor inline logic (locked by tests/_baseline + test_agent_config).
#
# Each getter still reads the raw dict via `get_agent(agent_id)` so the
# `_ensure_seed_agents()` side-effect is preserved, then validates that dict.
# `None / [] / "default"` defaults mean "no restriction / use global default".


def _cfg(agent_id: str) -> AgentConfig:
    return AgentConfig.model_validate(get_agent(agent_id) or {})
...
def get_agent_feature(agent_id: str, feature_name: str, default: bool) -> bool:
    ...
    """
    return _cfg(agent_id).feature(feature_name, default)


def get_agent_tools(agent_id: str) -> list[str] | None:
    """Per-agent tool whitelist (agent.json `tools: [str]`). None = no restriction."""
    return _cfg(agent_id).tools


def get_agent_disallowed_tools(agent_id: str) -> list[str]:
    return _cfg(agent_id).disallowed_tools


def get_agent_model(agent_id: str) -> str | None:
    """Per-agent LLM model id (agent.json `model`). None/empty/absent = global default."""
    return _cfg(agent_id).model


def get_agent_effort(agent_id: str) -> str | int | None:
    return _cfg(agent_id).effort


def get_agent_max_turns(agent_id: str) -> int | None:
    return _cfg(agent_id).max_turns


def get_agent_permission_mode(agent_id: str) -> str:
    return _cfg(agent_id).permission_mode


def get_agent_hooks(agent_id: str) -> dict:
    return _cfg(agent_id).hooks


def get_agent_initial_prompt(agent_id: str) -> str | None:
    return _cfg(agent_id).initial_prompt


def get_agent_spawn_targets(agent_id: str) -> list[str] | None:
    """Allowed spawn_subagent targets (agent.json `spawn_targets: [str]`).

    None / absent = no restriction; `["*"]` is normalized to None; a specific
    list is a whitelist.
    """
    return _cfg(agent_id).spawn_targets
