...
async def _resolve_labels(dimension: str, keys: list[str]) -> dict[str, str]:
    """Batch-resolve IDs in the given dimension to display names.

    user_id   → users_index.name (COALESCE email, id)
    task_id   → tasks_index.name  (fallback: truncated id)
    agent_id  → agents/<id>/agent.json.name via agents_svc
    model     → model string is human-readable enough, return {}

    Missing rows are simply omitted — the caller falls back to the raw key.
    """
    keys = [k for k in {*keys} if k]
    if not keys or dimension == "model":
        return {}
    db = get_index_db()
    if dimension == "user_id":
        rows = await db.fetchall(
            f"SELECT id, COALESCE(name, email, id) AS label FROM users_index "
            f"WHERE id IN ({','.join('?' for _ in keys)})",
            keys,
        )
        return {r["id"]: r["label"] for r in rows}
    if dimension == "task_id":
        rows = await db.fetchall(
            f"SELECT id, name AS label FROM tasks_index "
            f"WHERE id IN ({','.join('?' for _ in keys)})",
            keys,
        )
        # Tasks may have empty names — fall back to the id in that case.
        return {r["id"]: (r["label"] or r["id"]) for r in rows}
    if dimension == "agent_id":
        from app.services.agent import agents_svc

        try:
            catalog = {a["id"]: a.get("name") or a["id"] for a in agents_svc.list_agents()}
        except Exception:
            catalog = {}
...
async def by_dimension(*, dimension: str, days: int = 30, limit: int = 20) -> list[dict]:
    """dimension in {model, user_id, agent_id, task_id}. Each item carries a
    human-readable `label` (resolved from users_index / tasks_index / agents
    catalog); when resolution fails the label falls back to the raw key so the
    ...
    labels = await _resolve_labels(dimension, keys)
    return [
        {
            "key": r["key"],
            "label": labels.get(r["key"]) or r["key"],
