...
async def list_users(*, q: str | None = None, role: str | None = None, status: str | None = None) -> list[dict]:
    ...
    for r in rows:
        ...
        out.append({
            ...
            # Registration-approval audit fields so the frontend can show
            # "submitted at / reviewed by / reject reason" next to pending rows.
            "self_registered": bool(prof.get("self_registered")),
            "registration_submitted_at": prof.get("registration_submitted_at"),
            "reviewed_by": prof.get("reviewed_by"),
            "reviewed_at": prof.get("reviewed_at"),
            "reject_reason": prof.get("reject_reason"),
        })
    return out
...
async def review_registration(
    *, operator: dict, uid: str, decision: str, reason: str | None = None
) -> dict:
    """Admin approves or rejects a pending registration.

    `decision`:
        - "approved": status pending|rejected → active (user can now log in)
        - "rejected": status pending → rejected (user sees ACCOUNT_REJECTED)

    Writes reviewed_by / reviewed_at / reject_reason to the profile for audit
    and updates both the file + the users_index cache.
    """
    if decision not in ("approved", "rejected"):
        raise APIError(400, ErrorCode.VALIDATION_ERROR, "decision 只能是 approved 或 rejected")
    paths = get_paths()
    profile = read_json(paths.user_profile(uid))
    if not profile:
        raise APIError(404, ErrorCode.RESOURCE_NOT_FOUND, "用户不存在")
    current_status = profile.get("status") or "active"
    # Guard: only review accounts that are actually pending or previously rejected.
    if current_status not in ("pending", "rejected"):
        raise APIError(
            400,
            ErrorCode.VALIDATION_ERROR,
            f"账号当前状态为 {current_status}，无法审批（仅支持 pending/rejected）",
        )
    new_status = "active" if decision == "approved" else "rejected"
    now = _now()
    before = {"status": current_status, "reject_reason": profile.get("reject_reason")}
    profile["status"] = new_status
    profile["reviewed_by"] = operator["id"]
    profile["reviewed_at"] = now
    profile["reject_reason"] = (reason or "").strip() if decision == "rejected" else None
    write_json(paths.user_profile(uid), profile)
    db = get_index_db()
    await db.upsert(
        "users_index",
    ...
    await audit(
        ...
        diff={
            "before": before,
            "after": {"status": new_status, "reject_reason": profile["reject_reason"]},
        },
    )
    return profile
    ...
    for d in paths.tasks.iterdir():
        ...
        for jl in (d / "conversations").glob("*.jsonl") if (d / "conversations").exists() else []:
            ...
            try:
                ...
                pass
    from app.services.agent.agents_svc import list_agents
