    ...
    rollupOptions: {
      output: {
        manualChunks(id) {
          ...
          // Markdown stack: carved out so it loads only when MarkdownRenderer
          // actually renders (dynamic import boundary in MarkdownRenderer.tsx).
          // NOTE: the old "circular ESM" issue was caused by refractor/prismjs
          // (syntax highlighter) — removed in D1a. react-markdown alone is safe.
          if (
          ...
          // Shiki syntax highlighter: loaded async by CodeHighlighter.tsx.
          // Includes TextMate grammars + oniguruma WASM — never in first paint.
          if (
            id.includes("/shiki/") ||
            id.includes("/@shikijs/") ||
          ...
          // ECharts: large charting library (~300KB gzip), only admin pages use it.
          if (
            id.includes("/echarts/") ||
