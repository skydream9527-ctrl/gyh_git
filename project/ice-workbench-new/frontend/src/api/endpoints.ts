...
export const taskApi = {
  ...
  quickCreate: (message: string, agent_id?: string) =>
    api<{ task_id: string; agent_id: string; conversation_id: string; name: string }>(
      http.post("/tasks/quick-create", { message, agent_id }),
    ),
  remove: (id: string) =>
...
export const searchApi = {
  ...
  fulltext: (q: string, opts?: { task_id?: string; source_type?: string; limit?: number }) =>
    api<{ items: FulltextResult[]; total: number; query: string }>(
      http.get("/search/fulltext", {
        params: { q, task_id: opts?.task_id, source_type: opts?.source_type, limit: opts?.limit },
      }),
    ),
};
