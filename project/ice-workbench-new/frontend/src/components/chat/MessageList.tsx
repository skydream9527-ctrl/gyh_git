...
export function MessageList({
  ...
  // 把 finalized + partial 合并成「轮次组」
  const turns = useMemo(() => buildTurns(finalized, partial), [finalized, partial]);

  // --- Virtualizer: only renders visible turns + overscan ---
  const virtualizer = useVirtualizer({
    count: turns.length,
    getScrollElement: () => scrollRef.current,
    estimateSize: () => 120, // rough average; measureElement corrects it
    overscan: 5,
    // Allow items at end to be measured dynamically (streaming partial changes height)
    measureElement: (el) => el.getBoundingClientRect().height,
  });

  // --- Scroll tracking: detect sticky-bottom ---
  useEffect(() => {
  ...
  // --- Sticky-bottom: scroll to last item when stuck ---
  useLayoutEffect(() => {
    if (!stickRef.current || turns.length === 0) return;
    // scrollToIndex uses the virtualizer's measurement to land precisely
    virtualizer.scrollToIndex(turns.length - 1, { align: "end" });
  }, [turns.length, partial?.content, partial?.toolCalls.length, phase, runEvents.length]);
      ...
      <div className="msg-list" ref={scrollRef}>
        ...
        >
          {virtualItems.map((virtualRow) => {
            const t = turns[virtualRow.index];
            return (
              <div
                key={virtualRow.key}
