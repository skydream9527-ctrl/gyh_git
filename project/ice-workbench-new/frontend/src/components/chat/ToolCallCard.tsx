...
/** Elapsed time hook — ticks every second while active. */
...
export function ToolCallCard({ call, onRetry, onCopyError }: Props) {
  ...
  // Progress info from backend (injected in tool_call_start frame)
  const progressHint = (call as any).progress_hint as string | undefined;
  const estimatedSec = (call as any).estimated_sec as number | undefined;
      ...
      {isExecuting && (
        <div className="tool-progress">
          <div className="tool-progress-bar">
            <div
              className={`tool-progress-fill${estimatedSec ? "" : " indeterminate"}`}
              style={
                estimatedSec
                  ? { width: `${Math.min(95, (elapsed / estimatedSec) * 100)}%` }
                  : undefined
              }
            />
          </div>
          {progressHint && <span className="tool-progress-hint">{progressHint}</span>}
        </div>
      )}
      {showArgs && (
