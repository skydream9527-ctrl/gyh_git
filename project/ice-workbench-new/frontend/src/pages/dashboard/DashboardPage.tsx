...
import { kbApi, taskApi } from "@/api/endpoints";
...
import type { AgentCard, FileMeta, TaskSummary } from "@/types/api";
...
export function DashboardPage() {
  ...
  // --- React Query: auto-dedup, cache, background refetch ---
  const { data: agentsRaw = [], isLoading: agentsLoading } = useAgentList();
  const agents = sortAgents(agentsRaw);
  const { data: skills = [], isLoading: skillsLoading } = useSkillList();
  const { data: tasks = [], isLoading: tasksLoading } = useMyTasks();
  const { data: publicTasks = [] } = usePublicTasks();
  const { data: publicFiles = [] } = usePublicFiles();
  const { data: kbs = [] } = useKBList();
  const { data: scheduledItems = [] } = useMyScheduled();

  const loading = agentsLoading || skillsLoading || tasksLoading;
  ...
  const quickSubmit = async () => {
    ...
    try {
      const res = await taskApi.quickCreate(msg);
      navigate(`/workspace/${res.task_id}`);
    } catch (err) {
      pushToast("error", (err as Error).message || "快速创建失败");
        ...
        {activeTab === "quick" && (
          <section className="dash-quick-panel" aria-labelledby="dash-quick-title">
            <div className="dash-quick-head">
              ...
              <p>输入需求直接开始对话，或选择一个 Agent 创建任务。</p>
            </div>
            ...
            <div className="quick-chat-box">
              ...
              <button
                ...
                onClick={quickSubmit}
                disabled={!quickMsg.trim() || quickLoading}
                aria-label="发送"
              >
                {quickLoading ? (
                  <span className="quick-chat-spinner" />
                ) : (
                ...
                {agents.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="pc-desc">{a.user_friendly_description || a.description}</div>
                      {!comingSoon && (
                        <div className="pc-actions">
                          <button
                            type="button"
                            className="pc-action-btn pc-action-start"
                            onClick={() => startWith(a)}
                            disabled={!!starting}
                          >
                            立即开始
                          ...
                          <button
                            type="button"
                            className="pc-action-btn pc-action-detail"
