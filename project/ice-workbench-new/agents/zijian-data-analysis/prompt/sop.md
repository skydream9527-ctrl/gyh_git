...
1. 解析输入。若含飞书链接，先 `read_skill(skill_id="feishu")`，再用 `execute_python` 调 `feishu fetch` 读取（**禁止 WebFetch 飞书**，无认证必失败）。
2. 提 1~3 个澄清问题，覆盖：
...
| 飞书取链接 | `read_skill("feishu")` + `execute_python` 调 `feishu fetch` | 文档原文 |
