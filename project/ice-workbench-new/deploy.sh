...
if [ ! -f .env ]; then
  if [ -f .env.example ]; then
    cp .env.example .env
    # .env 不随发布包打包（含 LLM / 飞书 / Aegis 密钥）。这里自动生成一个强随机
    # ICE_SECRET_KEY 写进 .env —— app/main.py 会拒绝用 .env.example 的占位密钥启动，
    # 不生成的话紧接着的 `from app.main import app` 路由自检就会崩。
    GEN_SECRET="$(python3 -c 'import secrets; print(secrets.token_urlsafe(48))')" \
      python3 - "$ROOT/.env" <<'PY'
import os, re, sys
path = sys.argv[1]
secret = os.environ["GEN_SECRET"]
with open(path, "r", encoding="utf-8") as f:
    lines = f.readlines()
replaced = False
for i, ln in enumerate(lines):
    if re.match(r"\s*ICE_SECRET_KEY\s*=", ln):
        lines[i] = "ICE_SECRET_KEY=%s\n" % secret
        replaced = True
        break
if not replaced:
    lines.append("ICE_SECRET_KEY=%s\n" % secret)
with open(path, "w", encoding="utf-8") as f:
    f.writelines(lines)
PY
    warn ".env not found — copied from .env.example and auto-generated a random ICE_SECRET_KEY."
    warn "  → 起服务前请在 .env 填 LLM / 飞书 凭证（MIFY_GATEWAY_API_KEY / FEISHU_APP_ID 等）。"
  else
