...
| 数据平台指标/维度/数据集/数据表/业务域 | `data-platform-mcp` | 指标管理、维度管理、数据集管理、数据表管理、元数据配置、业务域、版本管理、指标一致性检测 |

## Skill 详情

### `data-platform-mcp`

- **用途**：调用数据平台 MCP 服务，管理指标、维度、数据集、数据表、业务域、元数据配置和版本，以及指标一致性检测。
- **触发**：指标、维度、数据集、数据表、业务域、元数据、版本管理、指标一致性、数据平台、数据资产。
- **依赖**：内网可达 `data-platform-mcp.mib.miui.com`，无需额外鉴权。
- **读取**：`read_skill(skill_id="data-platform-mcp")`
