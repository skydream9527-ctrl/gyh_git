...
[![pipeline status](https://gitlab.com/your-group/ice-workbench/badges/main/pipeline.svg)](https://gitlab.com/your-group/ice-workbench/-/pipelines)
...
`.env` **不随 `make pack` 打包**（含 LLM / 飞书 / Aegis 凭证）。解压后首次 `./deploy.sh` 会自动从 `.env.example` 生成 `.env` 并写入随机 `ICE_SECRET_KEY`，操作员只需补充 `MIFY_GATEWAY_API_KEY` 等业务密钥即可启动。本地开发直接复制 `.env.example` → `.env` 后填 key 同样可行。

## 跨机部署

> 📖 完整的服务端部署说明（必填环境变量、热更新不丢数据、部署后验证、特性开关灰度、排查表）见 [`deploy/README.md`](deploy/README.md)。
...
`make pack` 会剔除 `.venv` / `node_modules` / `.cache` / 运行时用户数据，同时**排除 `.env`**（安全：密钥不随包扩散），但**保留** `frontend/dist` 与 `.env.example`，解压后 `./deploy.sh --prod` 自动建 `.env` + 生成 `ICE_SECRET_KEY`。`DATA_ROOT` 默认解析到解压目录，无需手工改 .env。
...
- [`frontend/`](frontend/) — React 18.3 + Vite + TypeScript + Zustand
- [`agents/`](agents/) [`skills/`](skills/) [`files/`](files/) [`users/`](users/) [`tasks/`](tasks/) — G3 文件优先存储数据目录

### 🚀 部署运维

- [`deploy/README.md`](deploy/README.md) — **服务端部署说明**：必填环境变量、部署步骤、热更新不能丢的数据、部署后验证、特性开关灰度、排查表
- [`deploy/STAGING_CHECKLIST_agent_kernel.md`](deploy/STAGING_CHECKLIST_agent_kernel.md) — 统一 agent_kernel 迁移上线前的 staging 验证清单
- [`docs/SELF_HOSTING.md`](docs/SELF_HOSTING.md) — Docker Compose 自托管部署指南
...
| `/workspace/:taskId` 三栏 + 流式对话 + 多轮 Tool Calling（默认 20 轮，系统设置可调） | ✅ |
...
| 启动种子（管理员 + 测试用户） | ✅ |
| 语音输入 / 输出（ASR + TTS via MiMo，flag `ICE_VOICE_ENABLED`，默认关） | ✅ |
| HITL 人工介入（`request_human_input` 工具 + 任务级审批队列） | ✅ |
| 公共任务协作（join request 申请 / 审批 / 加入） | ✅ |
| 任务邀请（invitations，`/tasks` + `/me`） | ✅ |
| 任务意图识别自动选 Agent（task_intent） | ✅ |
| 登录 / 注册限流（rate_limit + 磁盘锁定兜底） | ✅ |
| 后台任务（`run_background` + bg_task 队列） | ✅ |
| Agent 私有工作流索引（`/workflow` 注入） | ✅ |
| 用量与成本（LLM 单价 / 月度预算 / CSV 导出，D125-D128） | ✅ |
| 经验卡片审批闭环（批准后注入 Agent，D118） | ✅ |
| 公共任务审核流（D122） | ✅ |
| Admin 诊断页（索引一致性 / 系统自检） | ✅ |
| 内置工具集（now / kyuubi_query / execute_python 沙盒 / write_file / read_file / list_files / feishu_publish / feishu_send_message / feishu_upload_image / memory_* / task_state_save / todo_write / spawn_subagent / create_scheduled_task / run_background / read_agent_knowledge / read_skill / data_platform_call 等 ~23 个） | ✅ |
...
|---|---|---|
| `/login` | 公开 | 登录 |
| `/auth/feishu/callback` | 公开 | 飞书 OAuth 回调（SPA 把 code 转发后端） |
| `/introduce` | 公开 | 产品介绍 |
| `/dashboard` | user+ | 任务首页 |
...
| `/admin/users/:userId/tasks` | admin+ | 查看指定用户的任务 |
...
- WebSocket subprotocol token 升级（D46，P2）+ 长会话稳定性
- 移动端适配 / 虚拟滚动 / ECharts 5 维统计
- 两个 `coming_soon` Agent 落地：经营洞察（biz-insight）/ 波动归因（wave-attribution）

> 备注：LLM 单价 / 月度预算 / SQL CSV 导出（D125-D128）、经验卡片审批闭环（D118）、公共任务审核流（D122）已实现，见上方「已实现」与「路由总览」。
