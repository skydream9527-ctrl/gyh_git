...
| 内容中心DAU/日活 | app_open, app_open_v2 |
| 内容中心信息流DAU/日活 | content_item_expose |
| 内容中心信息流人均VV | content_item_video_play, content_item_video_over, content_item_video_auto_play, content_item_video_auto_over |
| 内容中心信息流人均时长 | app_duration_v2, page_duration, content_duration, content_duration_v2 |
| 内容中心信息流人均消费时长 | page_duration, content_item_view, content_item_video_over, content_duration, content_duration_v2 |
| 内容中心信息流次日留存率 | app_open, app_open_v2 |
| 内容中心信息流消费DAU/日活 | content_item_click, content_item_view, content_item_video_play |
...
**关联指标**: 内容中心信息流人均时长, 内容中心信息流人均消费时长
...
**关联指标**: 内容中心信息流消费DAU/日活, 内容中心信息流人均消费时长
...
**关联指标**: 内容中心信息流人均VV, 内容中心信息流消费DAU/日活
...
**关联指标**: 内容中心信息流人均VV, 内容中心信息流人均消费时长
**关联页面**: 首页/推荐页, 首页, 沉浸式详情页, 短剧详情页
...
|--------|--------|------|------|
| ​common key | 公共属性 | string |  |
| from_page | 上级页面 | string | 从内容中心首页搜索框产生的搜索，from_page=newhome 从搜索结果页换query产生的搜索，from_page=search_result |
...
**上报时机**: 只要进入或回到搜索首页都记，包括从内容中心内其他页面进入或返回，以及从系统内其他地方进入（点击桌面icon进入、从其他app返回、其他app调起、拉回通知栏、从多任务进入、屏幕解锁、点击桌面书签进入）
...
**上报时机**: 1、item漏出三分之一记曝光（图片icon+文字一起算三分之一） 2、我的书签最多曝光前20个 3、如下不曝光：未在视线范围内出现的（包括键盘挡住的）、隐藏了的卡片、未展开的搜索历史 4、如下反复操作不重复曝光：页面反复上下滑动、展开收起（搜索历史）、隐藏展开（我的书签和经常访问）、键盘收起弹出、 5、滑动过程中不曝光，停止了才曝光 6、只要再次回到该页面都进行重新曝光，包括从内容中心内其他页面进
...
**上报时机**: 1、sug页每刷新一次即上报一次，即考虑连续输入 2、只要再次回到该页面都进行重新曝光，包括从内容中心内其他页面进入或返回，以及从系统内其他地方进入（点击桌面icon进入、从其他app返回、其他app调起、拉回通知栏、从多任务进入、屏幕解锁、点击桌面书签进入）
...
**上报时机**: 前端上报 1、item漏出三分之一记曝光（图片icon+文字一起算三分之一） 2、我的书签最多曝光前20个 3、如下不曝光：未在视线范围内出现的（包括键盘挡住的） 4、如下反复操作不重复曝光：页面反复上下滑动、键盘收起弹出 5、滑动过程中不曝光，停止了才曝光 6、只要再次回到该页面都进行重新曝光，包括从内容中心内其他页面进入或返回，以及从系统内其他地方进入（点击桌面icon进入、从其他app返回、其
...
| card_type | 卡片类型
（卡片的一级分类） | string | app：游戏 / 应用 product：小米有品 / 小米商城 video：小米视频 book：内容中心小说   website：网址   box：快递/ 彩票/ 天气 sugword：sugword  |
| item_type | item类型
...
| duration_type | 时长类型 | string | 从任何场景发起搜索开始计时，离开sdk结束计时： quit：直接home退到后台 home：返回内容中心界面（非百度sdk页面） add_window：新建窗口 lock：锁屏 push：点击push跳 |
...
| imei2 | imei2md5值 | string |  (为方便之后与内容中心做映射) |
...
| dp_ext | dp拓展字段 | string | has_guide&source_&pic_id_&is_ad_&channel_id_&father_channel_id_&ext_&url_&doc_id (如果来自deeplink方式，需打上dp的渠道号以及素材号，newhome-aaa) |
