# Browser - Mapping Table Schema

## Table Overview

| # | Table Name | Full Name | Description |
|---|-----------|-----------|-------------|
| 1 | dim_browser_did_label_di | did标签表 | date+did唯一粒度的标签打平表 |
| 2 | dm_browser_multi_dimension_indicators_di | 多维指标聚合表 | 浏览器核心指标多维度聚合日表 |
| 3 | dm_browser_multi_dimension_retain_indicators_di | 留存计算用did粒度指标表 | did粒度指标表，通过自关联计算留存 |
| 4 | ads_browser_user_type_core_indicators_di | 用户类型核心指标表 | 用户日活主题大数打平表 |
| 5 | ads_browser_item_type_core_indicators_di | 内容转化主题核心指标表 | 体裁维度内容转化大数打平表 |
| 6 | ads_browser_page_indicators_di | 小场景指标表 | 页面级指标聚合表 |
| 7 | ads_browser_finance_core_indicators_di | 财收核心指标表 | 广告收入相关指标聚合表 |

---

## 1. dim_browser_did_label_di（did标签表）

**Database**: `iceberg_zjyprc_hadoop.browser`
**粒度**: date + did 唯一

```sql
CREATE TABLE iceberg_zjyprc_hadoop.browser.dim_browser_did_label_di (
  date INT COMMENT '日期',
...
## 2. dm_browser_multi_dimension_indicators_di（多维指标聚合表）
...
## 3. dm_browser_multi_dimension_retain_indicators_di（留存计算用did粒度指标表）
...
## 4. ads_browser_user_type_core_indicators_di（用户类型核心指标表）
...
## 5. ads_browser_item_type_core_indicators_di（内容转化主题核心指标表）
...
## 6. ads_browser_page_indicators_di（小场景指标表）
...
## 7. ads_browser_finance_core_indicators_di（财收核心指标表）
