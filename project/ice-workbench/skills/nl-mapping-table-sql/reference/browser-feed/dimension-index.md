...
### dm_browser_multi_dimension_indicators_di（多维指标聚合表）
...
> **信息流筛选**：此表为浏览器整体财收表，查询信息流数据时需通过 `tag_id IN (...)` 限定信息流广告位（共41个），详见 commerce-metrics-reference.md 中的广告位筛选条件。

| Dimension | Field | Type | Description | Query Type |
|-----------|-------|------|-------------|------------|
| 日期 | date | INT | 分区字段 | 必选筛选 |
| 广告位场景 | ad_position_scene | STRING | 图文详情页/视频详情页/小视频详情页/短视频频道 | 枚举值查询 |
| 广告位 | tag_id | STRING | 广告位标识，信息流需限定41个tag_id | 枚举值查询+信息流筛选 |
...
|-----------|-------|-------------|-------------|
| 实验组 | exp_id | dwm_browser_event_aggregation_label_di | 实验分组 |
| 三方调起包名 | third_packagename | dwm_browser_event_aggregation_label_di | 三方调起来源包名 |
| 是否冷启 | is_coldstart | dwm_browser_event_aggregation_label_di | 冷启动标记 |
| 是否有上划行为 | is_feed_sliding | dwm_browser_event_aggregation_label_di | 信息流上划行为标记 |
| 内容源 | source | dwm_browser_event_aggregation_label_di | 内容来源 |
