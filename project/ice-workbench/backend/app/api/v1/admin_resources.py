...
@router.get("/knowledge-bases/{kb_id}/articles")
async def admin_kb_articles(kb_id: str, _: dict = Depends(require_admin)):
    items = kb_svc.list_articles(kb_id)
    return ok({"items": items, "total": len(items)})


@router.get("/knowledge-bases/{kb_id}/articles/{article_id}")
async def admin_kb_article_detail(
    kb_id: str, article_id: str, _: dict = Depends(require_admin)
):
