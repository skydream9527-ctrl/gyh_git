...
@router.get("/task/{task_id}/{file_id}/download")
async def download_file(task_id: str, file_id: str, user: dict = Depends(get_current_user)):
    """Raw bytes download. Works for both text and binary; browser saves as filename."""
    ...
    if not meta:
        ...
        if imp_meta:
            ...
            return FileResponse(fpath, filename=imp_meta.get("filename") or f"{file_id}.md")
        raise APIError(404, ErrorCode.RESOURCE_NOT_FOUND, "文件不存在")
    rel = paths.root / meta["path"]
    if not rel.exists():
        raise APIError(404, ErrorCode.RESOURCE_NOT_FOUND, "文件已被删除")
    return FileResponse(rel, filename=meta.get("name") or rel.name)
