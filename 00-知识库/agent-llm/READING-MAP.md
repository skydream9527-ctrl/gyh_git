...
> 顺序：先读 OVERVIEW 建立"一张表 + 两条流水线"的心智，再学骨架（基础检索），再看花活（Self-RAG / CRAG / GraphRAG），最后落地（LangGraph 实现）。

| 顺序 | 文件 | 解决什么 |
|---|---|---|
| 1 | [rag/OVERVIEW.md](rag/OVERVIEW.md) | 开篇导读：解决/不解决什么、知识库"长什么样"（一张表）、7 部件 + 两条流水线、部署三档、设计决策链、10 项 Checklist |
| 2 | [rag/retrieval-basics.md](rag/retrieval-basics.md) | 分块、向量化、混合检索、rerank、评测——RAG 的"骨架" |
| 3 | [rag/advanced-rag.md](rag/advanced-rag.md) | Self-RAG / CRAG / GraphRAG / Adaptive RAG 的机制和取舍 |
| 4 | [rag/agentic-rag-implementation.md](rag/agentic-rag-implementation.md) | CRAG / Self-RAG 用 LangGraph 写出可跑代码 + 叠加用法 |
| 5 | [rag/RAG原理与实现-以SQL需求库为例.md](rag/RAG原理与实现-以SQL需求库为例.md) | 原理+实战：6 问串讲(结构/模型/存储/切分向量化/检索/优化),以本地 sql-kb 项目代码逐一印证 |
