...
- 工具 → [tools-protocols/function-calling.md](tools-protocols/function-calling.md)、[tools-protocols/tool-development.md](tools-protocols/tool-development.md)（API 工程师视角的 schema/校验/错误处理）、[tools-protocols/mcp.md](tools-protocols/mcp.md)
...
- Agent 循环架构 → [agents/agent-loop-architectures.md](agents/agent-loop-architectures.md)（感知-行动 vs 状态机 vs ReAct 三种循环 + 嵌套用法）
...
- Skill vs Tool → [agents/skills-vs-tools.md](agents/skills-vs-tools.md)（本质区别 + 七个维度对比 + 协同设计示例）
...
- 延迟优化 → [production/latency-optimization.md](production/latency-optimization.md)（7 个降延迟杠杆 + 50% 降幅组合方案）
...
- 面试核心题（手写 ReAct / MCP 辨析 / 记忆系统） → [agents/interview-essentials.md](agents/interview-essentials.md)（三道高频题的追问深挖）
- 现代 GPT Block（Agent 工程师视角的 LLM 底层） → [llm-fundamentals/modern-gpt-block.md](llm-fundamentals/modern-gpt-block.md)（Pre-Norm / RMSNorm / RoPE / SwiGLU / GQA 五大改造 + 选模型检查表）
- RoPE 与位置编码 → [llm-fundamentals/rope-and-positional-encoding.md](llm-fundamentals/rope-and-positional-encoding.md)（旋转位置编码原理 + 长度外推 PI/NTK/YaRN + 5 个工程坑）
- MQA / GQA 与 KV Cache → [llm-fundamentals/mqa-gqa-and-kv-cache.md](llm-fundamentals/mqa-gqa-and-kv-cache.md)（KV cache 原理 + GQA 实战 + 长 context 内存估算公式）
...
### 路径 D：LLM 底层架构（给 Agent 工程师，理解性能边界）

```
1. Transformer 直觉    →  [llm-fundamentals/transformer.md](llm-fundamentals/transformer.md)（attention / QKV / KV cache 基础）
2. 现代 GPT Block      →  [llm-fundamentals/modern-gpt-block.md](llm-fundamentals/modern-gpt-block.md)（Pre-Norm / RMSNorm / RoPE / SwiGLU / GQA 五大改造）
3. RoPE 与位置编码     →  [llm-fundamentals/rope-and-positional-encoding.md](llm-fundamentals/rope-and-positional-encoding.md)（长度外推 + 5 个工程坑）
4. MQA/GQA + KV Cache  →  [llm-fundamentals/mqa-gqa-and-kv-cache.md](llm-fundamentals/mqa-gqa-and-kv-cache.md)（长 context 内存估算公式）
5. 推理优化            →  [llm-fundamentals/inference-optimization.md](llm-fundamentals/inference-optimization.md)（量化 / 投机解码 / Flash Attention / vLLM）
6. 落到实战            →  [production/latency-optimization.md](production/latency-optimization.md)（把架构知识转成 50% 延迟降幅）
```

适合：选模型 / 估成本 / 调延迟时希望"看得见底"的工程师。读完这 6 篇，**看一眼模型 spec 就能预判它在你 Agent 场景的表现**。
