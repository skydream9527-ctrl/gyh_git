...
function OwnerView({
  ...
  const submitReview = async (
    ...
    try {
      ...
      pushToast("error", extractErrMsg(err, "操作失败，请稍后重试").message);
    } finally {
      setActingId(null);
    }
  };

  const onApprove = (req: JoinRequest) => submitReview(req, "approved");
  const onAskReject = (req: JoinRequest) => {
    setRejectFor(req.id);
    setRejectReason("");
  };
  const onConfirmReject = (req: JoinRequest) =>
    submitReview(req, "rejected", rejectReason.trim() || undefined);
        ...
        {items.map((req) => {
          ...
          return (
            <li key={req.id} className="jrp-item">
              ...
              {isRejecting && (
                <div className="jrp-reject-row">
                  <input
                    type="text"
                    className="jrp-reject-input"
                    placeholder="拒绝原因（可留空）"
                    value={rejectReason}
