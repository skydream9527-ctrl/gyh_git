...
export function AdminKnowledgeBases() {
  ...
  const [browseFor, setBrowseFor] = useState<KBRecord | null>(null);
  ...
  return (
    <div>
          ...
          {items.map((kb) => (
            <div key={kb.id} className="adm-section" style={{ padding: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "flex-end" }}>
                ...
                <button className="btn-ghost" onClick={() => setBrowseFor(kb)}>
                  📂 浏览文档
                </button>
      ...
      {browseFor && <BrowseArticlesModal kb={browseFor} onClose={() => setBrowseFor(null)} />}
      <ConfirmModal
        open={!!confirmDelete}
...
function BrowseArticlesModal({ kb, onClose }: { kb: KBRecord; onClose: () => void }) {
  const pushToast = useUIStore((s) => s.pushToast);
  ...
  const openArticle = async (a: KBArticle) => {
    ...
    try {
      const full = await adminKBApi.article(kb.id, a.id);
      setSelected(full);
      // cache back into list so re-selecting is instant
                  ...
                  <li key={a.id}>
                    <button
                      ...
                      style={{
                        ...
                        background:
                          selected?.id === a.id ? "var(--primary-dim)" : "transparent",
                        color: "var(--text)",
                        fontSize: 12,
                        cursor: "pointer",
                      }}
