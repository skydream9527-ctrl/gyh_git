      ...
      {totalAlert > 0 && (
        <div className="adm-alert-card">
          ...
          <div className="adm-alert-list">
            {budgetAlert && alerts?.budget && (
              <div
                className="adm-alert-item"
                style={
              ...
              >
                <span>💰</span>
                {budgetAlert === "exceeded"
                  ? `本月 LLM 支出已超出预算：$${alerts.budget.cost_usd.toFixed(2)} / $${alerts.budget.budget_usd.toFixed(2)}（仅提醒，未停服）`
        ...
        {stats ? (
          <>
            ...
            <Stat
              label={
                alerts?.budget
                  ? `本月成本 / $${alerts.budget.budget_usd.toFixed(0)} 额度`
                  : "本月成本"
              }
              val={alerts?.budget ? `$${alerts.budget.cost_usd.toFixed(2)}` : "—"}
              color={
                budgetAlert === "exceeded"
                  ? "var(--error, #ef4444)"
                  : budgetAlert === "warning"
