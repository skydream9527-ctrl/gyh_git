...
| 4 | dm_newhome_user_type_core_indicators_di | 用户类型核心指标表 | 用户日活主题大数打平表 |
| 5 | dm_newhome_item_type_core_indicators_di | 内容转化主题核心指标表 | 体裁维度内容转化大数打平表 |
| 6 | dm_newhome_page_indicators_di | 小场景指标表 | 页面级指标聚合表 |
| 7 | dm_newhome_finance_core_indicators_di | 财收核心指标表 | 广告收入相关指标聚合表 |
...
## 4. dm_newhome_user_type_core_indicators_di（用户类型核心指标表）

**Database**: `iceberg_zjyprc_hadoop.newhome`

```sql
CREATE TABLE iceberg_zjyprc_hadoop.newhome.dm_newhome_user_type_core_indicators_di (
  date INT COMMENT '日期',
...
## 5. dm_newhome_item_type_core_indicators_di（内容转化主题核心指标表）

**Database**: `iceberg_zjyprc_hadoop.newhome`

```sql
CREATE TABLE iceberg_zjyprc_hadoop.newhome.dm_newhome_item_type_core_indicators_di (
  date INT COMMENT '日期',
  item_type STRING COMMENT '体裁',
  expose_pv BIGINT COMMENT '曝光量',
  expose_uv BIGINT COMMENT '曝光用户数',
...
## 6. dm_newhome_page_indicators_di（小场景指标表）

**Database**: `iceberg_zjyprc_hadoop.newhome`

```sql
CREATE TABLE iceberg_zjyprc_hadoop.newhome.dm_newhome_page_indicators_di (
  date INT COMMENT '日期',
...
## 7. dm_newhome_finance_core_indicators_di（财收核心指标表）

**Database**: `iceberg_zjyprc_hadoop.newhome`

```sql
CREATE TABLE iceberg_zjyprc_hadoop.newhome.dm_newhome_finance_core_indicators_di (
  date INT COMMENT '日期',
  app_port STRING COMMENT '应用端口',
