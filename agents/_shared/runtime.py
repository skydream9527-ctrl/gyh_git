...
class RuntimeFacade:
    def __init__(
        ...
        # workspace_root 指向 ice-workbench 仓库根，用于读 users/*/memory 与 tasks/*/STATE.md。
        # 未提供时按 Context Protocol，三层 memory 全部读空串，行为向后兼容。
        self.workspace_root = workspace_root

        # Memory tools are available to every agent by default. Register them
        # first so agent-specific tools can override (by name) if needed.
        self.tool_registry.register_many(build_memory_tools())
    ...
    async def handle_message(
        ...
        # Bind the current-request ContextPaths so memory tools (memory_save /
        # memory_delete / task_state_save) and _load_context can both pick it up.
        # Only bind when we actually have a workspace_root configured; otherwise
        # leave it None so memory tools correctly error out.
        paths_token = None
        if self.workspace_root:
            paths_token = set_current_paths(
                ContextPaths(
        ...
        try:
            ...
            # - No skill restriction → expose every registered tool.
            # - Skill restricts via `allowed_tools` → use that list, but always
            #   union `always_available` tools (memory / task state) so
            #   cross-cutting persistence capabilities survive skill scoping.
            skill_tools = self.skill_engine.get_allowed_tools(matched_skills)
            has_tools = bool(self.tool_registry.list_names())
            if skill_tools:
                essentials = self.tool_registry.list_always_available_names()
                # dict.fromkeys dedupes while preserving first-seen order.
                tool_names = list(dict.fromkeys(skill_tools + essentials))
            else:
            ...
            user_memory, agent_memory, task_state = self._load_context()
            ...
            if paths_token is not None:
                reset_current_paths(paths_token)
            self.trace_collector.finalize(trace)
            yield {"type": "done", "trace": trace.to_dict()}

    def _load_context(self) -> tuple[str, str, str]:
        """Load the three context layers from the current-request ContextPaths.

        Relies on `set_current_paths()` having been called earlier in
        `handle_message`. Returns empty strings when no paths are bound or
        any of the source files are missing.
        """
        paths = get_current_paths()
        if paths is None:
            return "", "", ""
        loader = ContextLoader(paths)
        try:
            return (
                loader.load_user_memory(),
                loader.load_agent_memory(),
                loader.load_task_state(),
            )
